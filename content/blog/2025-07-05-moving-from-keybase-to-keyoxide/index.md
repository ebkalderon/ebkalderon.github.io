...
social network which aimed to make [OpenPGP](https://www.openpgp.org/) and
approachable, convenient, and _fun_. By creating and uploading a
<abbr title="Pretty Good Privacy">PGP</abbr> public key to Keybase, you could:
...
It seems that Keyoxide automatically creates user profiles for any and all
public keys it can find on the major public PGP keyservers, such as
...
that is trivial enough to fix.

### Adding Identity Proofs

Adding new identity proofs to my Keyoxide profile is far less straightforward
