...
third-party text editors and IDEs. This project has been very interesting and
enlightening for me to work on because language servers tend to have very strong
...
parsing with an `Err(nom::Err::Error(_))` or `Err(nom::Error::Failure(_))` was
not a good idea, given the design constraints described above. I looked into
`Err(nom::Error::Incomplete(_))`, but found it similarly useless for my purposes
since it too causes the parser to abort early as soon as it encounters
incomplete input. I needed some way to log that a non-fatal parse error had been
...
I will admit I explored and learned about many topics during this time, from
benchmarking with [`criterion`] to generating flamegraphs with
...
I had basically come to the conclusion that parser combinator libraries like
`nom` were simply not flexible enough to have good hand-crafted diagnostics in
conjunction with permissive fault-tolerant parsing, and that perhaps PEGs were
fundamentally ill-equipped to handle the requirements of this project.

But then I stumbled upon the ["Syntax error recovery in parsing expression
grammars" (2018)][paper] paper while scouring the ACM DL search engine for
interesting articles last night.
...
prominent author of [`rust-analyzer`], a cutting-edge language server for Rust,
and [`rowan`], a library for lossless [concrete syntax trees].
...
strategy for PEG would also be applicable in a parser combinator library like
`nom`. If you're interested in learning the specifics of the authors' strategy,
I would strongly recommend you read the entire paper for yourself.
...
nicely to PEG parsers (and in turn, parser combinators) without sacrificing
their declarative nature. It also provides a small library of handy recovery
expressions you can use in different situations to either emit high quality
errors or suppress them.
...
I should also add that I did not cover the pros and cons of [scannerless
parsing], of which `nom` is a well known example. This is a parsing strategy in
which there is no separate lexing stage which runs before the parsing stage,
transforming the source text into a stream of tokens. Instead, scannerless
parsers process individual characters or byte chunks directly, effectively
lexing and parsing in a single step. I personally prefer having a separate lexer
in my own projects, especially when working with programming languages, because
I've found they help produce higher quality diagnostics. Thankfully, `nom`
happens to also be a great tool for building fast and efficient lexers as well.
See my [`nix-lexer`] crate for an example of this.

[scannerless parsing]: https://en.wikipedia.org/wiki/Scannerless_parsing
[`nix-lexer`]: https://github.com/ebkalderon/nix-language-server/tree/8c8a7de932e79bcf556725da582f84a43cda75ec/nix-lexer

For this reason, I can't integrate the code I've written here directly into my
Nix parser. I will need to transfer the same basic design decisions over to work
with a custom `Tokens<'a>` input type for `nom` instead of with a
`LocatedSpan<&'a str, State<'a>>`.
...
Parsers are a very interesting niche in the world of computer science, and
implementing a parser of your own with good error recovery strategies and rich,
user-friendly diagnostics is as much an art as it is a science (I think the Rust
compiler devs might agree with that statement). This is even more true when it
comes to parsers catering for the needs of language servers, REPLs, and other
interactive uses where you need to be very tolerant to parse errors and provide
meaningful diagnostics in response to variable, messy input. I learned many
valuable things on this journey, and I'm still learning further as I go along (I
